...
- 2026-09-16: Two bugs found on day 2 of the run and fixed with a 25-minute restart: milestone checkpoints never fired (boundary check on the wrong step), and resume loaded the 4.3GB checkpoint onto the GPU next to the live model and OOMed. Both in Chapter 4.
- 2026-09-20: Run finished at 20B tokens, 5 days 23 hours. Base model: HumanEval 13.4%, HumanEval+ 11.0%, MBPP 20.2%. SFT (82k examples, 93 minutes, one GPU): 26.8% / 25.6% / 38.9%. Decision: release the 20B checkpoint as version 1 rather than continue to 30B; the curves still have slope but the chat model is the deliverable and the marginal gain is a few points for three more days. Chapters 5, 7 and 8 written; chat server (scripts/serve.py) added.
- 2026-09-20: Two harness bugs caught before they became published numbers: HumanEval+ needs a 4GB sandbox (1GB gave 3.0% from MemoryError), and MBPP in chat mode must show the test assertions or every function is misnamed (2.5% without them).
