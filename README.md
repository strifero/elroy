...
The published model, Elroy-350M, is a 24-layer Llama-style decoder trained on about 20B tokens of educational Python and English on two workstation GPUs over about eight days. It is not a replacement for a frontier coding assistant. It writes short, correct Python functions, explains beginner concepts, and completes code in the middle of a file. The point is that you can see every line that made it do that.
...
| Two GPUs | Two 20GB+ cards with NVLink or fast PCIe | The published Elroy-350M run in about eight days. |

If you only want to use Elroy, skip to Chapter 8 and download the weights.
...
| 5 | [Pretraining Elroy](docs/05-pretraining.md) | The real run, with loss curves, evals, and everything that went wrong |
...
| 8 | [Release](docs/08-release.md) | Exported weights, a model card, and an inference-only script |
